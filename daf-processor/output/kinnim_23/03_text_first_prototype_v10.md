...
This concern is not abstract. The second group belongs to someone else — call him the second owner — who has no way of knowing which of his five birds is the intruder. If the first owner, by his own act of bringing all three of his remaining birds, automatically renders the flying bird a fixed *olah*, the second owner may then unknowingly bring that bird as a *chatat*. He cannot identify it; that is precisely the problem. One must therefore avoid any action that fixes the flying bird's identity and thereby restricts or burdens the second owner, who is entirely without recourse.
...
It is worth pausing to articulate the general principle that this case illustrates. Every time a bird flies into a group, it invalidates one bird from that group; every time a bird flies out, it invalidates one more. Each crossing, in other words, costs exactly one bird. In this scenario each group experienced one bird flying in and one flying out, a loss of two per group, leaving two usable birds from each. This pattern — lose one each time a bird enters, lose one each time it leaves — has a certain intuitive appeal as a general mathematical rule, and the subsequent *mishnayot* do indeed operate on that assumption. It is worth noting, however, that this principle will not always hold so cleanly, as the next *mishnah* will make clear.
