...
The conclusion is: *kav'ah lah veset le-yamim u-le-kfisot* — she has established a *veset* based on the combination of the day and the exertion. The *gemara*'s insistence on the combination reflects something important: neither the external stimulus alone nor the calendar alone is sufficient. The *gemara* understands that the exercise triggers bleeding only when the woman's body is itself prepared — when the internal biological readiness and the external prod coincide. This is why *ones* alone cannot establish a *veset*: a *veset* must track something that inheres in the woman's own body, not merely in external conditions. It is worth noting, though, that the *gemara*'s commitment to this biological intuition is not unlimited. Rashi accepts a *veset* combining calisthenics with a weekly Sunday cycle — a cadence that does not correspond to any real menstrual biology — which already pulls the analysis in a more formalist direction, a tension that becomes explicit in the cases Rav Ashi raises shortly.

Crucially, none of this affects the woman's status as a *niddah*. The *ones* analysis bears only on whether the pattern can anchor a *veset*; the bleeding itself, however it was occasioned, renders her *teme'ah* in full.
...
Beyond the twice-daily requirement, the *mishnah* adds that a woman must also check before and after sexual relations — *umishameshet be'edim*. The logic is straightforward: one must not engage in intercourse while in a state of *niddah*, and checking is the mechanism that ensures she has not become one in the interim. As noted in the previous section's discussion of the *gemara*, this pre- and post-intercourse checking likewise applies only during the period when people were actively concerned with *tum'ah* and *taharah*, and is not current practice.
