#!/usr/bin/env python3
"""
prototype_text_first_v9.py — PROTOTYPE. v6 (no lateness penalty) plus the colon fix.

v7/v8's lateness penalty fixed Berakhot 31b's header-traversal problem but damaged Gittin
18b (unrelated Gemara pulled into the opening; the tail stripped of its text). Human
review rated v6 'almost perfect' on Gittin and 90-95% on Berakhot with 'not so noticeable'
misses, so v6 is the safer default; the penalty remains available in v8 for dafim where the
traversal problem is observed. This file is v6 with one unambiguous improvement: the
colon/continuation fix.
...
def process(daf_dir: Path, threshold: float, diagnose: bool) -> dict | None:
    ...
    if not diagnose:
        ...
        out = daf_dir / "03_text_first_prototype_v9.md"
