...
### Bug 5 — orphan segmentation macros (no heading of their own in 03_final.md) shifted every later pill (found on Shabbat 12, fixed 2026-08-18)

**The bug:** reported as "each pill takes you to the next one" on Shabbat 12. More generally: on any daf where `01_segmentation.json`'s `macro_segments` has an entry that never got its own `## ` heading in `03_final.md`, pill navigation for that daf silently misjumps.

**Root cause:** `find_sefaria_indices.py` only trusts positional macro↔heading matching (`use_positional = len(matches) == len(macros)`) when the counts are equal. `macro_segments` (pass 1) and `03_final.md`'s actual `## ` headings (pass 2/rewrite, and — since the Approach B corpus-wide push — v10/relocate/wall-of-text) are independently generated and can drift: pass 2 sometimes folds a segmentation topic's content into a neighboring section's prose instead of giving it its own heading (a brief early mention later given full treatment as a `"(II)"` section is the most common shape; **Bava Batra 153**'s orphan, `"Found Document"`, was a stale leftover in the JSON from the fabricated-tail section manually deleted from the `.md` files back on 2026-08-07 — see that section above). Once counts mismatch for a daf, *every* macro in it falls onto the exact-title-string fallback (`title_to_hebrews.get(title) or title_to_hebrews.get(display)`), and the orphan macro's lookup fails outright, carrying forward the previous macro's `sefaria_index` — shifting that pill (and, if other macros in the same daf also fail the fragile string match, subsequent ones too) off its real target.

**Two distinct shapes of macro/heading mismatch exist — only one is what this bug fix addresses:**
- **Orphan macro** (`macros > headers`, the extra macro has zero heading presence at any level): the fixable case below.
- **Duplicate/extra headings** (`headers > macros`, or a heading appears twice): a *different*, already-documented bug (see "Pass 2 self-invents a 'see above' placeholder for a duplicate heading" above) — deleting a macro entry doesn't fix it. Left untouched by this fix; `fix_orphan_macros.py` reports these as `WEIRD` and refuses to touch them.

**Decision (user, 2026-08-18): approach B — delete the orphan pill rather than have it best-effort-land near its content.** There are no live users yet (still in testing), and not every topic needs its own nav pill — the content isn't missing, it's just uncredited in the navigation. The alternative (approach A — make `find_sefaria_indices.py`'s fallback alignment-tolerant so the pill lands as close as possible) was considered and explicitly rejected in favor of exactness: every remaining pill maps to something real and distinct.

**Fix — `fix_orphan_macros.py`** (new, daf-processor/): scans for macro/heading count mismatches, classifies each into `clean` (orphan macro, safe to remove) vs `weird` (duplicate/extra headings, needs the other fix), and with `--apply` removes clean orphans from `macro_segments`. **Does not simply discard an orphan's `micro_segments`** — each micro-segment's own title/display_title is checked against the essay's `### ` headings; if a match is found, the micro-segment is re-homed into whichever surviving macro's block that heading now falls under, rather than deleted. This matters: `find_amud_b.py` locates the amud A/B boundary by finding the nearest heading to the first amud-B Sefaria quote and looking up that heading's *exact* title in `macro_segments`/`micro_segments` (`lookup_segment()`) — naively deleting the whole orphan macro deletes that lookup target even when its heading is still live in the essay under a different macro. Caught exactly this on **Menachot 81**: removing macro `"Yitzchak Temura"` (11 of the daf's 16 macros were orphans, all genuinely folded into two surviving `## ` sections — confirmed by reading the essay text, not just counting) broke `find_amud_b.py` because the boundary-marking micro-segment `"Temura Violation"` (21:42) lived only inside that removed macro, even though `### Temura Violation` is still a real heading in the essay, now under `"Backup Without Oblig (II)"`. Re-homing fixed it without needing to touch anything else.

**Run 2026-08-18**: corpus-wide scan found 30 dafim with a macro/heading count mismatch; 24 were the clean orphan-macro case (Menachot 81 lost 11 of 16 pills — an outlier; most lost exactly 1) and were fixed. The other 6 (`menachot_20/72/73/75`, `sanhedrin_85`, `zevachim_67`) are the duplicate/extra-heading case and were left untouched. Verified no content lost or duplicated (diffed old vs. new micro-segment title sets per daf; every dropped micro-segment title has zero heading presence in the essay at any level, and no surviving macro ended up with duplicate micro-segment titles). Re-ran `find_sefaria_indices.py --force` and `find_amud_b.py --force` for all 24: sefaria-index matching succeeded cleanly on all 24 (counts now match, so positional matching applies again); amud-b detection succeeded on 23 — the 24th, **Nazir 44**, MISSes, but this was confirmed **pre-existing and unrelated** to this fix (reproduces identically when `find_amud_b.py --force` is run against Nazir 44's untouched pre-fix backup; its segmentation's own top-level `amud` field is the unreliable `"b"` despite a separate `nazir_44b` directory existing, and its previously-stored `amud_b_segment_index` pointed at 02:37 into the shiur — implausibly early for a real boundary, so this was likely already a stale/flaky detection before today).

**Side-finding, not acted on:** `fix_orphan_macros.py`'s classifier compares against the *unique set* of `## ` heading texts, not the raw count — which caught 18 more dafim with a hidden duplicate-heading bug that a naive count comparison misses entirely (raw header count happens to equal macro count only because one heading is duplicated and another is missing). These are all instances of the already-documented "self-invents a 'see above' placeholder" bug, just a fuller list than the 4 originally found. Not fixed here — out of scope for this session, left for whoever picks up that bug next: `nazir_36`, `nedarim_3`, `nedarim_86`, `niddah_44`, `rosh_hashanah_6`, `sanhedrin_40`, `shabbat_70`, `shabbat_75`, `shabbat_89`, `shabbat_90`, `shabbat_91`, `sotah_9`, `sukkah_54`, plus others not individually recorded here — re-run `python fix_orphan_macros.py` (report-only, no `--apply`) to regenerate the full current list.

**Follow-up required, not done in this session (no Supabase credentials in this environment):** the fixed `01_segmentation.json` files are committed to the repo, but the live `shiur_content.segmentation` column in Supabase still has the old (pill-mismatched) data for these 24 dafim until `python upload_to_supabase.py --dir output/<daf>` is run for each, from an environment with `SUPABASE_URL`/`SUPABASE_KEY` configured.
